...
7. **"Seen empty" must differ from "not seen"** (user call, 2026-07-07): the floor is **bright** (`COLORS.floor` 0x9db4b8, emissive 0x8fa7ab @ 0.5 — the user chose bold contrast over a subtle trace after playing both), and sky/fog (`COLORS.air` 0x07090c) sit above the decay target (`COLORS.dark` 0x020304). Measured levels: unscanned ≈ 3, swept air ≈ 7, swept floor (fresh) ≈ 20, live floor ≈ 150. Bonus: the bright-floor/dark-sky **horizon** is now a stable orientation and tilt-reading reference. The trace still **decays with memory** (~3 s) on purpose — a permanent distinction would be a minimap by another name.
