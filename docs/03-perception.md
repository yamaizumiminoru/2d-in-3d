...
**Hairline is the default since 2026-07-13** (user: 「1ピクセルの方が2次元人の体感っぽくていい」); key `4` toggles back to the wide strip for comparison. Key `5` toggles **zero afterimage** — the canvas is wiped every frame and only the live line exists.

**ADOPTED as story structure (user, 2026-07-13): afterimage is a LEARNED skill.** ST1 plays entirely in the zero-afterimage state (`level.memoryLocked`): anchor 1 is found by sound; each collect's reveal flash shows the next anchor (placed inside the flash's ~±38° view of the approach heading — `level.revealDuration` lengthens ST1's flash to 0.5 s since it is that stage's only map); **winning the stage unlocks memory** ("…and the world begins to remain") — from ST2 on, the panorama exists and is understood as the automated form of what the player was doing in their head. Rationale (user): if memory unlocks early, players never learn to use sound — audio literacy is acquired *before* visual memory, which structurally attacks the "solvable without sound" gap. Key `5` remains as the dev toggle for this state.
