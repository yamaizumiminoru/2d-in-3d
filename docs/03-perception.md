...
7. **"Seen empty" must differ from "not seen"** (user call, 2026-07-07): rendered emptiness (`COLORS.air` 0x07090c for sky/fog + the floor's glow) sits slightly *above* the decay target (`COLORS.dark` 0x020304), so scanned-empty leaves a faint trace over unscanned black — measured levels: unscanned ≈ 3, swept air ≈ 7, swept floor ≈ 9, live floor ≈ 24 (mean 8-bit luminance). The distinction **decays with memory** on purpose — a permanent distinction would be a minimap by another name. Keep `air > dark` and keep the gap subtle.
