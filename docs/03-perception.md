...
4. **Branch — Reveal** (`game.revealTimer > 0`): full 3D at alpha `0.18 + (timer/REVEAL_DURATION) * 0.62`, plus a green wash; timer counts down by `dt`. The strip below still draws — the reveal *overlays* perception, then evaporates back into it.
