...
- Decision ② (ping mode) is now **playable as an A/B/C test**: keys `0`/`1`/`2` switch periodic / on-demand / hybrid live; focused ping on `F` / right-click. Awaiting the user's ears. Also fixed: collecting an anchor in DEV 3D froze the game (reveal timer now lives in the loop).
