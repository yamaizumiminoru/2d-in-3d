...
- **Gamepad (2026-07-08, recommended controller):** left stick moves, right stick X = scan yaw (analog); **tilt is detented spring-hold on the shoulders** (R1/R2/R1+R2 = +30/45/60°, L-side mirrors, release sags back to vertical — decision ⑦). A jump, R3/X ping, L3/Y focus-toggle, B reset. Keyboard tilt = tap-step the same `0/±30/±45/±60` notches (↑/↓/wheel, persistent; ↑+↓ resets). Floating wireframe markers removed as clutter.
- **ITD added to tilt-stereo (2026-07-13):** the near ear now also hears *first* (≤0.7 ms interaural delay, same tilt law as the pan — verified L 0.14 / R 0.56 ms at 45° tilt). **Audio layers arrive one at a time:** ST1 is hum-only (`silentPings`); echoes join in ST2 — hum+echo at once is polyphony untrained ears can't split (user call).
- **Memory is learned (2026-07-13, ADOPTED + implemented):** ST1 plays with zero afterimage (`memoryLocked`) — anchor 1 by sound, then each reveal flash (0.5 s in ST1) shows the next anchor; the win unlocks memory. Audio literacy before visual memory, by design. Hairline (1 device px) scan is the default; keys 4/5 are dev toggles (wide strip / zero-afterimage). Dev server is now `tools/serve.py` (no-store — stale-module cache burned us).
- **Title arc (2026-07-08):** the start card's "COPLANAR" letters are coplanar at rest, scatter into distinct 3D angles on start (thrown into volume), and realign to coplanar on the win (the reunion) — the story told by the title. Audio auto-mutes when the tab is hidden.
- Dev hook: `window.coplanar` exposes `{player, game, pickups, beacons, solids}` plus `rawPad()` / `audioState()` for debugging.
