...
1. **High-contrast emissive colors only** — objects are mostly seen as afterimages; dark or desaturated geometry effectively doesn't exist. Use the `COLORS` palette (or the level's `palette`); every solid gets `addEdgeGlow` (edges survive fading longest). The canonical floor is bright — its horizon against the dark sky is an orientation/tilt reference — and dark-room pockets/stages darken *their own* floor via `palette` (ST8).
...
4. **Landmarks over maps.** Tall unique silhouettes (columns — floating wireframe markers were dropped 2026-07-08 as clutter) at asymmetric positions let players re-anchor their mental sketch after a fade-out. Every room needs ≥ 3 distinguishable landmarks at different bearings.
